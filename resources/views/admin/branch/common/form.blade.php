<div class="row">
    ...
    <div class="col-lg-4 col-md-6 mb-4">
        ...
        <select class="form-select" id="exampleFormControlSelect1" name="branch_head_id">
            ...
            @foreach($users as $key => $user)
                <option value="{{ $user->id }}" @selected( old('branch_head_id', isset($branch) && $branch->branch_head_id ) == $user->id)>{{ ucfirst($user->name) }}</option>
            @endforeach
        </select>
    </div>
    ...
    <div class="col-lg-4 col-md-6 mb-4">
        <label for="zkteco_ip" class="form-label">IP جهاز البصمة (ZKTeco)</label>
        <input type="text" class="form-control" id="zkteco_ip" name="zkteco_ip" value="{{ isset($branch) ? $branch->zkteco_ip : old('zkteco_ip') }}" autocomplete="off" placeholder="مثال: 192.168.1.201">
    </div>
