...
//    }
    ...
    public function payroll(Request $request): Factory|View|RedirectResponse|Application
    {
        try{
            ...
            if($request->all()){
                ...
                // --- مزامنة التأخيرات والإضافي من البصمات تلقائياً ---
                if (!empty($filterData['year']) && !empty($filterData['month'])) {
                    foreach ($payrolls as $payslip) {
                        $payslipId = is_object($payslip) ? $payslip->id : ($payslip['id'] ?? null);
                        $employeeId = is_object($payslip) ? $payslip->employee_id : ($payslip['employee_id'] ?? null);
                        $status = is_object($payslip) ? $payslip->status : ($payslip['status'] ?? null);

                        if (!$payslipId || !$employeeId || in_array($status, ['paid', 'locked', 'accepted'])) {
                            continue;
                        }

                        $attendanceService = app(\App\Services\Attendance\AttendanceService::class);
                        $attendanceData = $attendanceService->getEmployeeAttendanceDetailOfTheMonth([
                            'user_id' => $employeeId,
                            'year' => $filterData['year'],
                            'month' => $filterData['month'],
                            'date_in_bs' => AppHelper::ifDateInBsEnabled(),
                        ]);

                        $totalOvertimeMinutes = collect($attendanceData)->sum('overtime');
                        $totalUndertimeMinutes = collect($attendanceData)->sum('undertime');

                        $employeeSalary = \App\Models\EmployeeSalary::where('employee_id', $employeeId)->first();
                        $hourRate = $employeeSalary ? $employeeSalary->hour_rate : 0;
    ...
    public function printPayrollSheet(Request $request)
    {
        try {
            $this->authorize('view_payroll_list');
            $currency = AppHelper::getCompanyPaymentCurrencySymbol();
            ...
            foreach ($payrolls as $payslip) {
                ...
                if ($employeeId && in_array($status, ['paid', 'locked', 'accepted', 'generated', 'review'])) {
                    $employee = \App\Models\User::find($employeeId);
                    $bulkPayrollData[] = [
                        'name' => $employee ? $employee->name : 'غير معروف',
                        'basic_salary' => is_object($payslip) ? $payslip->basic_salary : ($payslip['basic_salary'] ?? 0),
                        'allowance' => is_object($payslip) ? $payslip->total_allowance : ($payslip['total_allowance'] ?? 0),
                        'overtime' => is_object($payslip) ? $payslip->overtime : ($payslip['overtime'] ?? 0),
                        'undertime' => is_object($payslip) ? $payslip->undertime : ($payslip['undertime'] ?? 0),
                        'absent' => is_object($payslip) ? $payslip->absent_deduction : ($payslip['absent_deduction'] ?? 0),
                        'advance' => is_object($payslip) ? $payslip->advance_salary : ($payslip['advance_salary'] ?? 0),
                        'ssf' => is_object($payslip) ? $payslip->ssf_deduction : ($payslip['ssf_deduction'] ?? 0),
                        'net_salary' => is_object($payslip) ? $payslip->net_salary : ($payslip['net_salary'] ?? 0),
                    ];
                }
            }

            return view($this->view.'print_payroll_sheet', compact('bulkPayrollData', 'currency', 'filterData'));
        } catch (\Exception $exception) {
